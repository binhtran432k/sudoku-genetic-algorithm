...
class Candidate:
    ...
    def __init__(self, Nd):
        self.Nd = Nd
        self.values = numpy.zeros((self.Nd, self.Nd), dtype=int)
        self.fitness = None
        return
    ...
    def update_fitness(self):
        ...
        row_count = numpy.zeros(self.Nd)
        column_count = numpy.zeros(self.Nd)
        block_count = numpy.zeros(self.Nd)
        row_sum = 0
        column_sum = 0
        block_sum = 0

        for i in range(0, self.Nd):  # For each row...
            for j in range(0, self.Nd):  # For each number within it...
                row_count[self.values[i][j]-1] += 1  # ...Update list with occurrence of a particular number.

            row_sum += (1.0/len(set(row_count)))/self.Nd
            row_count = numpy.zeros(self.Nd)

        for i in range(0, self.Nd):  # For each column...
            for j in range(0, self.Nd):  # For each number within it...
                column_count[self.values[j][i]-1] += 1  # ...Update list with occurrence of a particular number.

            column_sum += (1.0 / len(set(column_count)))/self.Nd
            column_count = numpy.zeros(self.Nd)


        # For each block...
        for i in range(0, self.Nd, 3):
            for j in range(0, self.Nd, 3):
                ...
                block_sum += (1.0/len(set(block_count)))/self.Nd
                block_count = numpy.zeros(self.Nd)
