...
class Candidate:
    ...
    def __init__(self):
        self.values = zeros((digitNumber, digitNumber), dtype=int)
        self.fitness = None
        return
    ...
    def update_fitness(self):
        ...
        row_count = zeros(digitNumber)
        column_count = zeros(digitNumber)
        block_count = zeros(digitNumber)
        row_sum = 0
        column_sum = 0
        block_sum = 0

        for i in range(0, digitNumber):  # For each row...
            for j in range(0, digitNumber):  # For each number within it...
                row_count[self.values[i][j]-1] += 1  # ...Update list with occurrence of a particular number.

            row_sum += (1.0/len(set(row_count)))/digitNumber
            row_count = zeros(digitNumber)

        for i in range(0, digitNumber):  # For each column...
            for j in range(0, digitNumber):  # For each number within it...
                column_count[self.values[j][i]-1] += 1  # ...Update list with occurrence of a particular number.

            column_sum += (1.0 / len(set(column_count)))/digitNumber
            column_count = zeros(digitNumber)


        # For each block...
        for i in range(0, digitNumber, 3):
            for j in range(0, digitNumber, 3):
                ...
                block_sum += (1.0/len(set(block_count)))/digitNumber
                block_count = zeros(digitNumber)
